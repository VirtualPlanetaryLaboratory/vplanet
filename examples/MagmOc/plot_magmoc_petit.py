...
r_sol_petit       = data_petit[:,3]  # solidification radius (R_earth)
M_water_mo_petit  = data_petit[:,4] # water mass in magma ocean + atmosphere (TO)
M_water_sol_petit = data_petit[:,5] # water mass in solid mantle (kg)
M_O_mo_petit      = data_petit[:,6] # mass of oxygen in magma ocean + atmosphere (kg)
M_O_sol_petit     = data_petit[:,7] # mass of oxygen in solid mantle (kg)
Press_H2O_petit   = data_petit[:,8] # partial pressure water in atmopshere (bar)
Press_O_petit     = data_petit[:,9] # partial pressure oxygen in atmosphere (bar)
M_H_Space_petit   = data_petit[:,10] # partial pressure oxygen in atmosphere (bar)
M_O_Space_petit   = data_petit[:,11] # partial pressure oxygen in atmosphere (bar)
Frac_Fe2O3_petit  = data_petit[:,12] # partial pressure oxygen in atmosphere (bar)
NetFluxAtmo_petit = data_petit[:,13] # atmospheric net flux (W/m^2)

n_time = len(time)
n_time_petit = len(time_petit)
i_end  = n_time-1

M_water_atm = np.zeros(n_time)
M_O_atm     = np.zeros(n_time)

M_water_atm_petit = np.zeros(n_time_petit)
M_O_atm_petit     = np.zeros(n_time_petit)
...
for i in range(n_time_petit):
    M_water_atm_petit[i] = Press_H2O_petit[i]*1e5 * 4 * np.pi * r_p**2 / g
    M_O_atm_petit[i]     = Press_O_petit[i]*1e5 * 4 * np.pi * r_p**2 / g
...
MagmOc_Vol_Time_petit = 0
for i in range(1,n_time_petit):
    MagmOc_Vol_Time_petit = MagmOc_Vol_Time_petit + 4./3 * np.pi * (r_p**3 - r_sol_petit[i]**3) * (time_petit[i]-time_petit[i-1])

print('Time integrated magma ocean volume (grey) = ' , MagmOc_Vol_Time,       ' m^3*yr')
print('Time integrated magma ocean volume (petit) = ', MagmOc_Vol_Time_petit, ' m^3*yr')
...
fig.text(x=0.5, y=0.02, s='Solid line = grey atmosphere, dashed line = petitCODE', fontsize=14, ha="center")
...
ax1.plot(time_petit*10**-6, Tpot_petit, linestyle='--', color=cmap(0))
ax1.plot(time_petit*10**-6, Tsurf_petit, linestyle='--', color=cmap(220))
...
ax2.plot(time_petit*10**-6, r_sol_petit/1.15, color=cmap(0), linestyle='--')
...
ax3.plot(time_petit*10**-6, M_water_atm_petit/TO, linestyle='--', color=cmap(0))
ax3.plot(time_petit*10**-6, M_water_mo_petit-M_water_atm_petit/TO, linestyle='--', color=cmap(120))
ax3.plot(time_petit*10**-6, M_water_sol_petit, linestyle='--', color=cmap(220))
...
ax4.plot(time_petit*10**-6, Press_H2O_petit, linestyle='--', color=cmap(0))
ax4.plot(time_petit*10**-6, Press_O_petit, linestyle='--', color=cmap(220))
...
ax5.plot(time*10**-6, Frac_Fe2O3, color=cmap(0))

ax5.plot(time_petit*10**-6, Frac_Fe2O3_petit, linestyle='--', color=cmap(0))

# ax5.legend(loc='best', frameon=True)
ax5.set_ylabel('$Fe_2O_3$ mass frac in magma ocean')
# ax5.set_yscale('log')
...
#
# ax6.plot(time_petit*10**-6, M_O_atm_petit, linestyle='--', color=cmap(0))
# ax6.plot(time_petit*10**-6, M_O_mo_petit-M_O_atm_petit, linestyle='--', color=cmap(120))
# ax6.plot(time_petit*10**-6, M_O_sol_petit, linestyle='--', color=cmap(220))
#
...
ax6.plot(time_petit*10**-6, NetFluxAtmo_petit, linestyle='--', color=cmap(0))
