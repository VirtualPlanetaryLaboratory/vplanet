...
Initial_water = 1
# read data
data = np.loadtxt("radheat2GJ1132.GJ1132b.forward")
data_sch = np.loadtxt("radschaeferGJ1132.GJ1132b.forward")
...
for i in range(n_time_sch):
    Tpot[i]        = Tpot[i] - Tpot_sch[i]
    Tsurf[i]       = Tsurf[i] - Tsurf_sch[i]
    r_sol[i]       = r_sol[i] - r_sol_sch[i]
    M_water_mo[i]  = M_water_mo[i] - M_water_mo_sch[i]
    M_water_sol[i] = M_water_sol[i] - M_water_sol_sch[i]
    M_O_mo[i]      = M_O_mo[i] - M_O_mo_sch[i]
    M_O_sol[i]     = M_O_sol[i] - M_O_sol_sch[i]
    Press_H2O[i]   = Press_H2O[i] - Press_H2O_sch[i]
    Press_O[i]     = Press_O[i] - Press_O_sch[i]
    M_H_Space[i]   = M_H_Space[i] - M_H_Space_sch[i]
    M_O_Space[i]   = M_O_Space[i] - M_O_Space_sch[i]
    Frac_Fe2O3[i]  = Frac_Fe2O3[i] - Frac_Fe2O3_sch[i]
    NetFluxAtmo[i] = NetFluxAtmo[i] - NetFluxAtmo_sch[i]
    Frac_H2O[i]    = Frac_H2O[i] - Frac_H2O_sch[i]
    RadioHeat[i]   = RadioHeat[i] - RadioHeat_sch[i]
...
print('Time integrated magma ocean volume (RadHeat) = ' , MagmOc_Vol_Time,       ' m^3*yr')
...
# fig.text(x=0.5, y=0.02, s='Solid line = Radiogenic heating with RadHeat, dashed line = Schaefer', fontsize=14, ha="center")
fig.text(x=0.5, y=0.02, s='Difference between Radiogenic heating with RadHeat and Schaefer', fontsize=14, ha="center")
