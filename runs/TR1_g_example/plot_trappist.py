...
K40 = 1
# Solidification time:
T_Solid  = [0,0,0,17.0485,15.285,15.4726]
T_Desicc = [0,0,0,31.114,64.7048,117.2161]
...
for i in range(n_time):
    ...
    diff_2 = abs(time[i] - T_Desicc[N_Planet-1]*1e6)
    if (diff < diff_old):
        n_t_solid = i
    diff_old = diff
    if (diff_2 < diff_old_2):
        n_t_desicc = i
    diff_old_2 = diff_2
...
ax2.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
ax2.set_ylim([0.5,1])
...
ax3.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
...
ax4.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
...
ax5.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
...
ax6.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
ax6.legend(loc='best', frameon=True)
ax6.set_ylabel('Oxygen Mass (kg)')
ax6.set_yscale('log')
ax6.set_ylim([1e17,1e21])
...
ax7.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
ax7.set_yscale('log')
...
ax8.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
...
ax9.axvline(x=T_Desicc[N_Planet-1],linestyle='--', color=cmap(140))
