...
void InitializeOptionsAtmEsc(OPTIONS *options, fnReadOption fnRead[]) {
  ...
  fvFormattedString(&options[OPT_XFRAC].cDescr,
                    "Fraction of planet radius in X-ray/UV");
  ...
  fvFormattedString(
        &options[OPT_ATMXABSEFFH].cLongDescr,
  ...
  fvFormattedString(
        &options[OPT_ATMXABSEFFH2OMODEL].cLongDescr,
  ...
  fvFormattedString(&options[OPT_WATERLOSSMODEL].cValues,
                    "LB15 LBEXACT TIAN LS16");
  ...
  fvFormattedString(
        &options[OPT_WATERLOSSMODEL].cLongDescr,
        "The water loss rate will be determined by the selected model.\n"
        "The options are LB15, LBEXACT, TIAN, and LS16.\n");

  fvFormattedString(&options[OPT_PLANETRADIUSMODEL].cName,
                    "sPlanetRadiusModel");
  fvFormattedString(&options[OPT_PLANETRADIUSMODEL].cDescr,
  ...
  fvFormattedString(
        &options[OPT_STOPWATERLOSSINHZ].cLongDescr,
  ...
  fvFormattedString(
        &options[OPT_BONDILIMITED].cLongDescr,
  ...
  fvFormattedString(
        &options[OPT_ENERGYLIMITED].cLongDescr,
  ...
  fvFormattedString(
        &options[OPT_RRLIMITED].cLongDescr,
  ...
  fvFormattedString(
        &options[OPT_ATMESCAUTO].cLongDescr,
  ...
  fvFormattedString(&options[OPT_HALTDESICCATED].cName,
                    "bHaltSurfaceDesiccated");
  fvFormattedString(&options[OPT_HALTDESICCATED].cDescr,
                    "Halt at Desiccation?");
  ...
  fvFormattedString(
        &options[OPT_THERMTEMP].cLongDescr,
  ...
  fvFormattedString(&options[OPT_FLOWTEMP].cDescr,
  ...
  fvFormattedString(&options[OPT_PRESXUV].cDescr,
  ...
  fvFormattedString(&options[OPT_ATMGASCONST].cDescr,
                    "Atmospheric Gas Constant");
  fvFormattedString(&options[OPT_ATMGASCONST].cDefault, "4124");
  fvFormattedString(&options[OPT_ATMGASCONST].cDimension,
      ...
      if (body[iBody].dAtmEscXi > 1) {
        ...
        if (!io->baRocheMessage[iBody] && io->iVerbose >= VERBINPUT &&
          ...
          fprintf(stderr,
                  ...
                  body[iBody].cName);
          fprintf(stderr,
                  "Consider setting bUseBondiLimited = 1 or bAtmEscAuto "
...
void SetEnergyLimitedEsacpe(BODY *body, UPDATE *update,
                            fnUpdateVariable ***fnUpdate, int iBody) {
...
void SetRRLimitedEsacpe(BODY *body, UPDATE *update,
                        fnUpdateVariable ***fnUpdate, int iBody) {
...
void SetBondiLimitedEsacpe(BODY *body, UPDATE *update,
                           fnUpdateVariable ***fnUpdate, int iBody) {
...
void AssignAtmEscDerivatives(BODY *body, EVOLVE *evolve, UPDATE *update,
  ...
  if (body[iBody].dEnvelopeMass > 0) {
    if (body[iBody].iHEscapeRegime == ATMESC_ELIM) {
      ...
      fprintf(stderr, "ERROR: Unknown envelope escape regime in "
                      "AssignAtmEscDerivatives!\n");
    }
  }
...
void VerifyEscapeRegime(BODY *body, CONTROL *control, int iBody) {
...
void VerifyAtmEsc(BODY *body, CONTROL *control, FILES *files, OPTIONS *options,
  ...
  if (body[iBody].iPlanetRadiusModel == ATMESC_LEHMER17) {
    ...
    for (iCol = 0; iCol < files->Outfile[iBody].iNumCols; iCol++) {
      ...
      if (strcmp(files->Outfile[iBody].caCol[iCol],
                 output[OUT_PLANETRADXUV].cName) == 0) {
        /* Match! */
        fprintf(stderr,
      ...
      //            strlen(files->Outfile[iBody].caCol[iCol])) == 0) {
      if (strcmp(files->Outfile[iBody].caCol[iCol],
                 output[OUT_SCALEHEIGHT].cName) == 0) {
        /* Match! */
        fprintf(stderr,
                "ERROR: Output option %s only allowed with AtmEsc's LEHMER17 "
                "model.\n",
  ...
  if (body[iBody].dEnvelopeMass > 0) {
    VerifyEscapeRegime(body, control, iBody);
  ...
  *dTmp = *(update[iBody].pdDEnvelopeMassDtAtmesc);
  ...
  if (output->bDoNeg[iBody]) {
    *dTmp *= output->dNeg;
    ...
    fvFormattedString(cUnit,
                      "kg/s"); // XXX Why isn't the other line commented out???
  }
}
...
void WriteRunawayGreenhouseFlux(BODY *body, CONTROL *control, OUTPUT *output,
                                SYSTEM *system, UNITS *units, UPDATE *update,
                                int iBody, double *dTmp, char **cUnit) {

  *dTmp = fdHZRG14(body, iBody);
...
void InitializeOutputAtmEsc(OUTPUT *output, fnWriteOutput fnWrite[]) {
  ...
  fvFormattedString(&output[OUT_OXYGENMASS].cDescr,
  ...
  fvFormattedString(&output[OUT_OXYGENMANTLEMASS].cDescr,
  ...
  fvFormattedString(&output[OUT_RGLIMIT].cDescr,
  ...
  fvFormattedString(&output[OUT_PRESSURF].cDescr,
  ...
  fvFormattedString(&output[OUT_PRESXUV].cDescr,
                    "Pressure at base of thermosphere");
  ...
  fvFormattedString(&output[OUT_ATMGASCONST].cDescr,
                    "Atmospheric gas constant");
  ...
  fvFormattedString(&output[OUT_RADSOLID].cDescr,
  ...
  fvFormattedString(
        &output[OUT_RRCRITICALFLUX].cDescr,
  ...
  fvFormattedString(&output[OUT_XH2O].cDescr,
  ...
  int iOut = OUT_RGFLUX;
  fvFormattedString(&output[iOut].cName, "RunawayGreenhouseFlux");
  fvFormattedString(&output[iOut].cDescr,
                    "Incident flux that triggers runaway greenhouse");
  fvFormattedString(&output[iOut].cNeg, "W/m^2");
  output[iOut].bNeg       = 1;
  output[iOut].dNeg       = 1;
  output[iOut].iNum       = 1;
  output[iOut].iModuleBit = 1;
  fnWrite[iOut]           = &WriteRunawayGreenhouseFlux;
...
int fbDoesWaterEscape(BODY *body, EVOLVE *evolve, IO *io, int iBody) {
  ...
  if (fbFloatComparison(dInstellation, -1) &&
      fbFloatComparison(body[iBody].bCalcFXUV, 0)) {
...
double fdXUVEfficiencyBolmont2016(double dFXUV) {
  ...
  if (dFXUV > 0) {
    ...
    if ((x >= -2) && (x < -1)) {
      ...
      dWaterEscapeEfficiency =
            pow(10, b0 * x * x * x + b1 * x * x + b2 * x + b3);
    } else if ((x >= 0) && (x <= 5)) {
      dWaterEscapeEfficiency =
            pow(10, c0 * x * x * x + c1 * x * x + c2 * x + c3);
...
double fdLopez12EnvelopeMassFromMassRadiusAge(BODY *body, SYSTEM *system,
