...
void PropsAuxLehmer17(BODY *body, int iBody) {
...
void PropsAuxAtmEscRadii(BODY *body, IO *io, int iBody) {
...
void PropsAuxXUVFlux(BODY *body, int iBody) {
  ...
  // to mass of H (MTG)
  // XXX No factor of 1.00784! RB 01/10/24
  ...
  } else { // Use Below for LBEXACT with MGs diff limited treatment XXX What
           // does comment mean?
    body[iBody].dFHRef =
...
void fnPropsAuxAtmEsc(BODY *body, EVOLVE *evolve, IO *io, UPDATE *update,
  ...
  /* This function should look like this:
    PropsAuxAtmEscGeneral()
    PropsAuxAtmEscEnvelopeEscape()
    PropsAuxAtmEscTerrestrialEscape()
  */


  if (body[iBody].iPlanetRadiusModel == ATMESC_LEHMER17) {
    PropsAuxLehmer17(body, iBody);
  }
  PropsAuxAtmEscRadii(body, io, iBody);
  ...
  double Xcompare = Xh2o; // the mixing ratio XFreeO must be equal to or greater
