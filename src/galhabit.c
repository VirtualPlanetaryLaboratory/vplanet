...
void VerifyGalHabit(BODY *body, CONTROL *control, FILES *files,
  ...
  if (iBody == 1) {
    ...
    if (system->bRadialMigr) {
      ...
      system->dScalingFTot =
            (dDMR + dStarR + dGasR) /
            system->dGalacDensity; //scale factor for total density (star+gas+dm)
...
void WriteDEccDtGalHTidal(BODY *body, CONTROL *control, OUTPUT *output,
  ...
  if (body[iBody].bGalacTides) {
    dex = *(update[iBody].padDEccXDtGalHabit
                  [0]); //safe to assume iEqn = 0 here, since tides
...
void WriteDIncDtGalHTidal(BODY *body, CONTROL *control, OUTPUT *output,
  ...
  if (body[iBody].bGalacTides) {
    dhx = *(update[iBody].padDAngMXDtGalHabit
                  [0]); //safe to assume iEqn = 0 here, since tides
...
void PropsAuxGalHabit(BODY *body, EVOLVE *evolve, IO *io, UPDATE *update,
  ...
  dMu = BIGG *
        (body[iBody].dMassInterior +
         body[iBody]
               .dMass); //calculate mass coefficient for primary/primary+secondary
