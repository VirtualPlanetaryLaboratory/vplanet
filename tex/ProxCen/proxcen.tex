...
same motion through the galaxy. \xxx{The} proper motion and radial
...
updated kinematic information, \cite{Anosova94} conclude that Proxima
...
HZ \xxx{\citep{Kopparapu13}}. More specifically, the planet receives about 65\% of Earth's
...
Tidal locking \xxx{led} many researchers to conclude that planets of M
...
processes described in this section to \xxx{the} full stellar system in order to
...
(dashed curves). In all cases $a$ and $e$ decrease and the \xxx{amount} of
...
energy is transformed \xxx{into} frictional heating inside the planet. The
bottom \xxx{panel} of Fig.~\ref{fig:eqtide} shows the average surface energy
...
In the previous section we outlined numerous possibilities for the history of Proxima b. We considered processes spanning the planet's core \xxx{evolution} to 
the \xxx{tidal torques from the} Milky Way galaxy and find that effects at all these scales could be important in the history of our closest exoplanet. In this 
section, we summarize the results in terms of the potential atmospheres that Proxima b might have, and which \xxx{are} considered in detail in 
\citep{Meadows16}. Then we examine the likelihood that it is currently habitable.
...
Regardless of whether Prxoima b spent \xxx{significant} time in a runaway
...
interior to the \xxx{HZ and} hence it may have developed a dense CO$_2$
...
levels to trigger the runaway greenhouse. \xxx{REF?}
