...
a cohesive model that can fold in the impact of the many factors that shape its evolutionary
...
percent, for high enough quality data \citep{Chaplin2014}. Indeed,
...
unequivocally that Proxima b is not a ``mini-Neptune.''
...
$e$ and $i$ the two methods are nearly indistiguishable.
 
\begin{figure}
\includegraphics[width=\textwidth]{orbcomp_mid_e_i.eps}
\caption{Comparison between the \distorb module and the N-body code and \footnotesize{\texttt{HNBody}} for one of our 2
...
bulge), with a shape controlled by the rotation rate \xxx{REF}. The planet then is
...
dependent on \distorb through the eccentricity, the
...
encounters \citep{Heisler1986, Heisler1987, Rickman2008} for the Oort
cloud (Prox Cen is arguably a high mass Oort cloud object, for
$\alpha$ Cen A and B).  We utilize an updated galactic density of
$\rho_0 = 0.102$ \citep{Holmberg2000} and treat $\alpha$ Cen A and B
as a single point mass with $M = 2.1$ M$_{\odot}$ (\ie the sum of the
updated masses given by \cite{PourbaixBoffin16}). This approach is
somewhat crude, as the two stars produce a significant quadrupole
moment associated with their orbits---the torque associated with this
quadrupole potential would be equal to the galactic tidal torque at
$\sim 2000$ AU. Hence, the effect of the binary host should be minor
though still important at Prox Cen's estimated distance of $~15,000$
AU, especially if the star has significant eccentricity. However, the
modeling of the triple system in a comprehensive way is sufficiently
complicated \citep[see, i.e.][]{Harrington1968, Ford2000} to place it
beyond the scope of this work.  Instead, we restrict ourselves to the
secular effects of galactic tides and passing stars and will revisit
the triple star dynamics in a future work.
...
planetary system, such as eccentricity excitation \citep{Kaib14}.  In such
situations, Proxima b may have significant tidal heating despite the
circularization time scale. \xxx{Figure on impulse approximation for b's eccentricity as a function of pericenter distance?}

Following \cite{Heisler1987} and \cite{Rickman2008}, we model stellar encounter
...
the solar \xxx{trq- not Proxima's?} apex velocity, \cite[see][]{Rickman2008}), and then a
...
approximation to assume axisymmetry of the total mass density, as in
\cite{Heisler1986}.  We scale the velocity
...
orbit \citep{Roskar2010}, hence the migration time is short compared to
...
$10^{-8}$~degrees. The bottom left panel shows the evolution of the
dynamical ellipticity as predicted by (\xxx{REF}). The lower right panel
shows the value of the Cassini Parameter \xxx{define in Section 2} for the two cases, both of
...
\xxx{dflemin3: Using the new default escape params in ATMESC and starting with 
dEnvelopeMass = -0.001, dSurfaceWaterMass = -4.5, I get ~1.04 Earth Oceans left at the end of my Envelope simulation}

\xxx{dflemin3: what is fig:qchoice? -- is that supposed to be fig:tidal_hec ?}
