...
Centauri's planetary system. If planet b has an additional
planet, as indicated by the radial velocity data, then it may perturb
...
find that some cases prevent the production of magnetic field, but
others do not. We develop a realistic model for the evolution of
Proxima Centauri and find that if planet b formed in situ, then it
experienced 150 million years in a runaway greenhouse as the star
dimmed during its formation. This increased brightness can permanently
desiccate the planet and/or produce a large abiotically-produced
oxygen atmosphere. On the other hand, if Proxima Centauri b formed
with a thin hydrogen atmosphere ($\lesssim~1$\% by mass), then it
could shield the water long enough for it to be retained before being
blown off itself. We therefore identify pathways for planet b to be
...
reached in an analysis of the RAVE survey by \citep{Kordopatis15}.  We
...
\citep[\eg][]{StorchLai14}. We will consider planets with very thing
hydrogen envelopes, so we will make this latter assumption use the $Q$
value computed by \thermint (see below) for core-dominated cases.
...
excess of this abundance. Note that Earth required tens to hundreds of millions of years to form, so all the primordial $^{26}Al$ in the Solar System had already decayed.
...
% Russell/Tom
Proxima is tenuously bound, if it is gravitationally bound at all, to 
...
for the Oort cloud (Proxima is arguably a high mass Oort cloud 
...
at Proxima's estimated distance of $~15,000$ AU, especially if 
...
\citep[see, \ie][]{Harrington1968, Ford2000} to place it beyond 
...
300$~W/m$^2$ \cite{Kasting93,Abe93} and for dry planets it is at
...
\caption{Distributions of trials in which Proxima's orbit about $\alpha$ Cen
  ...
  have Proxima pass within a few hundred AU of $\alpha$ Cen A and B.}
\label{fig:galacdist}
\end{figure}
...
side. Because of the pre-main sequence luminosity evolution of
...
loses 1 TO of water in $\lesssim 4$ Myr; if it formed with less water
...
but such scenarios are unlikely \emph{a priori} and we do not consider
them here.
...
are the HZ limits for dry planets of different albedos from \cite{Abe11}.}
\label{fig:HZEvol}
\end{figure}
...
hotter than thesurface of the Sun. This result is clearly unphysical,
and while some of the issue is that we began this case with a mantle
temperature of 3000~K, the real problem is that the heating from just
1 ppt of $^{26}$Al is so large that our model cannot handle
it. However, we do see that after this initial burst of heating, the
planet calms down into an Earth-like evolution. Thus, if heating from
$^{26}$Al is just a passing energy source, it may not affect the
evolution. On the other hand, the heating could be so large that the
planet may be irrevocably altered. At this time, we merely point out
that the influence of $^{26}$Al could be significant for planets with
formation times of order 1~Myr.
...
Next we add tidal heating to examine its role on the planet's interior
...
three different eccentricities of 0.05, 0.1 and 0.2. In
...
that the planet's tidal reponse is changes due to heating in the
interior, preventing the large tidal power productions expects by
...
like in the ``steady-state'' regime identified in
\cite{DriscollBarnes15} in which the planet cools very slowly as a
feedback develops in which the planet's $Q$, $a$ and $e$ acheive an
...
Although the mantles respond to the different tidal power rates, the
...
the inner core sizes are growing earlier and more rapidly than in the
...
early core nucleation. Although none of our cases acheive a fully
...
\begin{figure} 
\begin{center}
\includegraphics[width=0.9\textwidth]{tides.pdf}
\end{center}
...
\subsection{Habitable Evapoorated Core Scenarios}
...
\begin{equation}
\label{eqn:Q_hec}
\frac{k_2}{Q} = \frac{k_{2_{interior}}}{Q_{interior}} + \frac{ k_{2_{ocean}}}{Q_{ocean}} +
\frac{ k_{2_{envelope}}}{Q_{envelope}}
\end{equation}
where we remove $k_{2_{i}}/Q_i$ from the summation if we neglect the $i$th's
...
\ref{sec:results:orbital} and consistent with observations of Earth
today \citep{Dickey94,Williams78,Yoder95}.  This low tidal $Q$ is
caused by efficient energy dissipation by oceans.  The ``No Ocean''
...
initial inventory of 10 Earth oceans of water with $Q_{ocean} = 12$.
The full habitable evaporated core ``Envelope'' case considers a
planet with a hydrogen envelope that has an initial mass fraction of
$0.001$ of the planet's total mass with $Q_{envelope} = 10^4$ which is
consistent with measurements of Neptune's tidal Q
\citep{ZhangHamilton08}.  The Envelope case starts with just 4.5 Earth
oceans to demonstrate the envelope's ability to shield surface water
from atmospheric escape.  The results of the simulations are shown in
Figure \ref{fig:tidal_hec}.  We set $k_{2_{ocean}} = 0.3$ and
$k_{2_{enevelope}} = 0.01$ to let the thermal interior and oceans set
$k_2$ as these components dominate both the size and mass of the
planet.

The No Ocean case, dominated by the mantle, reaches tidal Qs of a few
$100s$ and undergoes minimal tidally-driven orbital evolution until
...
minimally to the initial tidal evolution and as the planet's radius
evolves with envelope mass.  Stellar UV flux completely strips the
...
initially shielded the planet's water allowing for enough to survive
subsequent photolysis such that about 1 Earth ocean of the initial 4.5
remained once the planet entered the HZ if we assume the planet has
...
\subsection{Habitable, but Dry}

As shown in Fig.~\ref{fig:HZEvol}, a dry planet would enter a potentially
habitable state earlier than an Earth-like planet, and hence it may be
that Proxima b is a dry, but habitable world. This could occur if the
planet formed {\it in situ} and the gas disk was able to shield the
planet from water loss. The habitable evaporated core scenario is also
a possibility if the core was dry and a thin layer of hydrogen could
be blown away. On the other hand, a planet without much water does not
need to spend much time in a runaway greenhouse to become
desiccated. For this latter reason, we conclude the dry and habitable
case to be unlikely, but our models cannot exclude it.
...
metabolisms; this may not be the case on Proxima b.
...
greenhouse. As shown in $\S$~\ref{sec:results:internal}, the
eccentricity of b will damp, but the timescale can be very long. Large
eccentricities are not well-modeled by equilibrium tide theory, even
with a proper accounting of geophysical features as in the
\thermint~module, so it is difficult at present to assess the role of
tidal heating in water retention.
...
particles. However, it is not clear that a magnetic field is always
beneficial for life, as it also increases the cross-sectional area of
the planet for charged particles and funnels the energy into the
magnetic poles, possibly increasing mass loss.
